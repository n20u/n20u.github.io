...
2. IoC容器
    ...
    - BeanFactoryPostProcessor可以在容器启动阶段读取并修改bean的配置元数据。
    - BeanPostProcessor可以在对象实例化阶段对其做处理。
    - 基于注解的容器配置：
        - 声明bean
            ```Java
...
3. Bean
    ...
    在IoC容器中，bean的定义表示为`BeanDefinition`实例。
...
7. Spring中的面向切面编程
    ...
    3. AOP代理
        - Spring AOP默认使用标准的 JDK 动态代理作为AOP代理，其可以代理目标类型实现的所有接口。
        - Spring AOP还可以使用 CGLIB 代理。当类没有实现任何接口时，必须用 CGLIB 代理对类进行代理。
    4. @AspectJ支持
